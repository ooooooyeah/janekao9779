...
// @author       ooooooyeah
// @match        https://x.com/*
// @updateURL    https://github.com/ooooooyeah/janekao9779/raw/main/auto-interaction.js
// @downloadURL  https://github.com/ooooooyeah/janekao9779/raw/main/auto-interaction.js
// @grant        none
// ==/UserScript==
...
(function () {
    ...
    style.textContent = `
        ...
        .ozx-comment-form textarea {
            ...
            margin-bottom: 12px;
            padding: 8px;
            border: 1px solid #F4C17F; /* 次要色调边框 */
            border-radius: 4px;
            font-size: 14px;
            resize: vertical;
            min-height: 80px;
        ...
        .ozx-comment-form textarea:focus {
            outline: none;
            border-color: #ED843F; /* 主色调聚焦效果 */
            box-shadow: 0 0 0 2px rgba(237, 132, 63, 0.2);
        }

        .ozx-comment-form label {
            display: block;
            margin-bottom: 6px;
            color: #8B6E4E; /* 基于主色调的深色文本 */
            font-size: 13px;
            font-weight: 500;
        }
        ...
        .ozx-comment-form button {
            ...
            transition: background 0.2s ease;
            width: 100%;
            text-align: center; /* 新增：使按钮文字居中 */
        }
        
        .ozx-comment-form button:hover {
            background: #d47536; /* 主色调加深 */
        }
        ...
        /* 模式按钮颜色方案 - 使用指定色卡 */
        .ozx-mode-toggle-btn[data-mode="comment"] { 
            background: #FCEEAE; /* 强调色：浅黄米色 */
        }
        .ozx-mode-toggle-btn[data-mode="quote"] { 
            background: #F4C17F; /* 次要色调：浅橙色 */
        }
        .ozx-mode-toggle-btn[data-mode="retweet"] { 
            background: #ED843F; /* 主色调：温暖橙色 */
            color: white; /* 主色调上的白色文本更清晰 */
        }
        
        .ozx-mode-toggle-btn:hover {
    ...
    configBtn.textContent = '推文互动';
    document.body.appendChild(configBtn);

    const form = document.createElement('div');
    form.className = 'ozx-comment-form';
    form.innerHTML = `
        <label>评论内容（两个换行分隔不同评论）</label>
        <textarea id="comment-content" placeholder="输入评论内容...
每个评论之间用两个换行分隔

保持单个换行会在评论中显示为换行"></textarea>

        <label>评论话题后缀（每行一个话题or关键词）</label>
        <textarea id="comment-suffix" placeholder="#话题1
#话题2
#话题3"></textarea>

        <button id="save-comment-btn">保存设置</button>

         <div style="color: #744322; font-size: 12px; margin-top: 12px; text-align: center;">@kaojanejanekao9779</div>
    `;
